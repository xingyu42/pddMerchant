...
**结论**：零依赖，`node:test` + 自研 `_harness.js`（mulberry32 PRNG + 生成器 + property runner）。

**Why**：
- `design.md §非目标`：不引入新的 npm 依赖。排除 `fast-check`。
- `mulberry32` 是一个 32-bit state 的快速确定性 PRNG，6 行可实现，足以覆盖枚举/属性样本的随机性需求；CI 可复现。
- 所有 property 用例在 `node:test` 的 `test()` block 内调用 `property()`，失败时抛普通 Error，与既有 15 个 `.test.js` 文件行为一致。

**不选**：
- `fast-check` — 新依赖；`node:test` runner 集成不如 jest/vitest 平滑。
...
- 所有 PBT 文件命名 `*.pbt.test.js`，由 `package.json` 的 `test` script 显式列入
