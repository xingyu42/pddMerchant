...
| 2026-04-23 | `85f0a123-f7e3` | _(pending)_ | docs(recon): V0.2 #2 goods_id endpoint 初步抓包 (受阻于账号风控) | 账号 445301049 vodka/goodsList 当日返 54001 风控；重新解读 V0.1 "goods_id 全 null" 结论可能本身是风控降级样本；2 个潜在候选 (glide/addProperties/goodsList, plutus/bin/goods/query) 被动响应空需后续 recon；recon 脚本落地支持 13 种 goods_id key 变体识别 | - | 低 |
| 2026-04-24 | `a7d23620-2128` | _(pending)_ | feat(v0.3): 架构审计落地 + V0.3.1 EndpointClient singleton 串联 | bundle V0.3 audit + V0.3.1 wire（依赖新文件无法用 git add -p 分离）；F-1/F-6/F-7 adapter port + token bucket + PageSession；F-4 withCommand 迁移 13 命令；mall-switcher 拆分为 mall-id/reader/writer/xhr-probe；F-2/F-5/F-8 logger stderr + errorToEnvelope + commander usage envelope；V0.3.1 rate-limiter-singleton 进程级 + run-endpoint 332→67 LOC shim + EndpointClient ctx.pageSession per-call (R1 修复)；_RATE_LIMIT_CONFIG 改 Object.defineProperties getter/setter (codex Critical QPS=0 + env 校验 / gemini Proxy 可读性) | - | 中 |
